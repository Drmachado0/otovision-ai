-- Correção runtime: evita arrays tipados para recorrencia_grupo_id e usa
-- subconsultas relacionais compatíveis com o tipo real da coluna legado.
...
BEGIN
  ...
  WHERE user_id = p_user_id AND deleted_at IS NULL
    AND recorrencia_mae = true AND recorrencia_ativa = true;
  ...
  WITH removed AS (
    ...
    WHERE occurrence.user_id = p_user_id
      ...
      AND occurrence.status = 'pendente'
      AND (
        occurrence.recorrencia_mae = true
        OR EXISTS (
          SELECT 1 FROM public.obra_transacoes_fluxo mother
          WHERE mother.user_id = p_user_id
            AND mother.recorrencia_mae = true
            AND mother.recorrencia_grupo_id IS NOT NULL
            AND mother.recorrencia_grupo_id = occurrence.recorrencia_grupo_id
        )
      )
  ...
  UPDATE public.obra_assistant_delegations SET last_used_at = now()

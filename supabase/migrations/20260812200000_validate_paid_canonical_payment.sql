-- Forward-only: valida data ISO e replays sobre transação canônica já paga.
  ...
  IF p_obligation_type IN ('purchase', 'purchase_installment') THEN
    ...
    IF p_obligation_type = 'purchase_installment' THEN
      ...
      IF NULLIF(v_installment->>'data_vencimento', '') IS NOT NULL
         AND NOT (v_installment->>'data_vencimento' ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$') THEN
  ...
  ELSE
    ...
    IF FOUND THEN
      IF v_existing_transaction.valor <> v_value
         ...
         OR lower(COALESCE(v_existing_transaction.status, '')) NOT IN ('pendente', 'pago')
      ...
      END IF;
      IF lower(v_existing_transaction.status) = 'pago' THEN
        IF v_existing_transaction.conta_id IS DISTINCT FROM p_account_id::TEXT
           OR v_existing_transaction.data_pagamento IS DISTINCT FROM p_paid_at
           OR trim(COALESCE(v_existing_transaction.metodo_pagamento, v_existing_transaction.forma_pagamento, '')) IS DISTINCT FROM trim(p_method)
           OR v_existing_transaction.comprovante_path IS DISTINCT FROM p_receipt_path THEN
          RAISE EXCEPTION 'Transação paga diverge da solicitação' USING ERRCODE = '23514';
        END IF;
      ELSE
        UPDATE public.obra_transacoes_fluxo
        SET status = 'pago', data_pagamento = p_paid_at,
            conta_id = p_account_id::TEXT,
            forma_pagamento = trim(p_method), metodo_pagamento = trim(p_method),
            comprovante_path = COALESCE(p_receipt_path, comprovante_path),
            updated_at = now()
        WHERE id = v_existing_transaction.id AND user_id = v_user_id;
      END IF;

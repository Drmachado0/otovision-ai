-- Fonte única, transacional e idempotente para pagamentos financeiros.
-- Valores e vínculos são obtidos da obrigação travada; o cliente informa apenas
-- conta, método, instante, política de comissão e uma chave idempotente.

CREATE TABLE IF NOT EXISTS public.obra_financial_payment_operations (
  ...
  transaction_id UUID REFERENCES public.obra_transacoes_fluxo(id) ON DELETE RESTRICT,
  commission_id UUID REFERENCES public.obra_comissao_pagamentos(id) ON DELETE RESTRICT,
  result JSONB NOT NULL DEFAULT '{}'::JSONB,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT obra_fin_payment_operation_key UNIQUE (user_id, idempotency_key),
  CONSTRAINT obra_fin_payment_installment_positive CHECK (installment_number IS NULL OR installment_number > 0)
);

ALTER TABLE public.obra_financial_payment_operations ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Users view own financial payment operations" ON public.obra_financial_payment_operations;
CREATE POLICY "Users view own financial payment operations"
  ON public.obra_financial_payment_operations FOR SELECT TO authenticated
  USING (auth.uid() = user_id);

-- Chave futura de deduplicação. Linhas históricas continuam preservadas e não
-- bloqueiam a criação do índice, pois só novas operações atômicas preenchem a chave.
ALTER TABLE public.obra_comissao_pagamentos
  ADD COLUMN IF NOT EXISTS dedup_key TEXT;

CREATE UNIQUE INDEX IF NOT EXISTS uq_obra_comissao_active_compra
  ON public.obra_comissao_pagamentos(user_id, dedup_key)
  WHERE deleted_at IS NULL AND origem_compra_id IS NOT NULL AND dedup_key IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS uq_obra_comissao_active_transacao
  ON public.obra_comissao_pagamentos(user_id, dedup_key)
  WHERE deleted_at IS NULL AND origem_compra_id IS NULL AND transacao_id IS NOT NULL AND dedup_key IS NOT NULL;
...
BEGIN
  ...
  IF p_obligation_type = 'transaction' THEN
    ...
    SET status = 'pago', data_pagamento = p_paid_at, conta_id = p_account_id,
  ...
  ELSE
    INSERT INTO public.obra_transacoes_fluxo(
      ...
      p_paid_at, v_category, v_description, trim(p_method), trim(p_method), p_account_id,
    ...
    SELECT jsonb_agg(
      CASE WHEN (item->>'numero')::INTEGER = p_installment_number
        THEN item || jsonb_build_object(
          ...
          'data_pagamento', p_paid_at, 'conta_id', p_account_id,
  ...
  ELSIF p_obligation_type = 'invoice' THEN
    ...
    SET status = 'Paga', data_pagamento = p_paid_at::DATE, conta_id = p_account_id,
